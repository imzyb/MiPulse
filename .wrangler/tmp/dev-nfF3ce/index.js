...
// .wrangler/tmp/bundle-1ejwdg/checked-fetch.js
...
// .wrangler/tmp/bundle-1ejwdg/strip-cf-connecting-ip-header.js
...
vps.get("/public", async (c) => {
  ...
  if (cache) {
    try {
      ...
      console.warn("[Cache Search Error]", e.message);
    }
  }
  try {
    console.log("[API] Fetching public nodes...");
    const { results: nodes } = await db.prepare(`
            SELECT id, name, region, countryCode, status, lastSeenAt, tag, lastReport, totalRx, totalTx 
            FROM vps_nodes 
            WHERE isPublic = 1 
            ORDER BY \`order\` ASC, name ASC
        `).all();
    const { results: latencyRows } = await db.prepare(`
            SELECT node_id, latencyMs, reportedAt 
            FROM (
                SELECT node_id, latencyMs, reportedAt,
                       ROW_NUMBER() OVER(PARTITION BY node_id ORDER BY reportedAt DESC) as rn
                FROM vps_reports
                WHERE reportedAt > datetime('now', '-24 hours')
            ) WHERE rn <= 20
        `).all();
    const latencyMap = {};
    latencyRows.forEach((r) => {
      if (!latencyMap[r.node_id])
        latencyMap[r.node_id] = [];
      latencyMap[r.node_id].push(r.latencyMs);
    });
    const response = c.json({ success: true, nodes: data });
    if (cache) {
      try {
        response.headers.set("Cache-Control", "public, max-age=60");
        c.executionCtx.waitUntil(cache.put(cacheKey, response.clone()));
      } catch (e) {
        console.warn("[Cache Search/Put Note]", e.message);
      }
    }
    return response;
  } catch (err) {
...
async function computeSignature(secret, nodeId, timestamp, payloadCanonical) {
  const data2 = `${nodeId}.${timestamp}.${payloadCanonical}`;
  ...
  const signature = await crypto.subtle.sign("HMAC", key, new TextEncoder().encode(data2));
...
vps.post("/report", async (c) => {
  ...
  if (!nodeId)
    return c.json({ error: "Missing x-node-id" }, 400);
  ...
  const lastReport = node.lastReport ? JSON.parse(node.lastReport) : {};
  const curRx = Number(report.traffic?.rx || 0);
  const curTx = Number(report.traffic?.tx || 0);
  const lastRx = Number(lastReport.traffic?.rx || 0);
  const lastTx = Number(lastReport.traffic?.tx || 0);
  ...
  const timeDelta = (currentTs - lastSeenTs) / 1e3;
  ...
  try {
    await db.batch([
      ...
      db.prepare(`
                INSERT INTO vps_reports (id, node_id, cpuPercent, memPercent, diskPercent, load1, latencyMs, reportedAt)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?)
            `).bind(
        crypto.randomUUID(),
        nodeId,
...
vps.get("/nodes", async (c) => {
  const db = c.env.MIPULSE_DB;
  try {
    const { results } = await db.prepare("SELECT * FROM vps_nodes ORDER BY `order` ASC").all();
    const data2 = results.map((n) => ({
      ...n,
      latest: n.lastReport ? JSON.parse(n.lastReport) : null
    }));
    return c.json({ success: true, nodes: data2 });
  } catch (err) {
    return c.json({ success: false, error: err.message }, 500);
  }
});
vps.post("/add", async (c) => {
  const db = c.env.MIPULSE_DB;
  const body = await c.req.json();
  const { name, region, isPublic, secret, order, tag } = body;
  const id = crypto.randomUUID();
  const finalSecret = secret || Math.random().toString(36).substring(2, 15);
  try {
    await db.prepare(`
            INSERT INTO vps_nodes (id, name, region, isPublic, secret, \`order\`, tag)
            VALUES (?, ?, ?, ?, ?, ?, ?)
        `).bind(id, name, region, isPublic ? 1 : 0, finalSecret, order || 0, tag || "").run();
    return c.json({ success: true, node: { id, secret: finalSecret } });
  } catch (err) {
    return c.json({ success: false, error: err.message }, 500);
  }
});
vps.post("/delete", async (c) => {
  const db = c.env.MIPULSE_DB;
...
vps.post("/update", async (c) => {
  const db = c.env.MIPULSE_DB;
  const body = await c.req.json();
  const { id, name, region, isPublic, order, tag, secret } = body;
  try {
    await db.prepare(`
            UPDATE vps_nodes SET 
                name = ?, region = ?, isPublic = ?, \`order\` = ?, tag = ?, secret = ?
            WHERE id = ?
        `).bind(name, region, isPublic ? 1 : 0, order || 0, tag || "", secret, id).run();
    return c.json({ success: true });
  } catch (err) {
    return c.json({ success: false, error: err.message }, 500);
  }
...
vps.get("/alerts", async (c) => {
  const db = c.env.MIPULSE_DB;
  try {
    const { results } = await db.prepare("SELECT * FROM vps_alerts ORDER BY createdAt DESC LIMIT 100").all();
    return c.json({ success: true, alerts: results });
  } catch (err) {
    return c.json({ success: false, error: err.message }, 500);
  }
});
vps.get("/node/:id", async (c) => {
  const db = c.env.MIPULSE_DB;
...
vps.post("/order", async (c) => {
  const db = c.env.MIPULSE_DB;
  const { orders } = await c.req.json();
  try {
    const statements = orders.map(
      (o) => db.prepare("UPDATE vps_nodes SET `order` = ? WHERE id = ?").bind(o.order, o.id)
    );
...
async function signing(privateKey, alg, data2) {
  ...
  return await crypto.subtle.sign(algorithm2, cryptoKey, data2);
}
__name(signing, "signing");
async function verifying(publicKey, alg, signature, data2) {
  const algorithm2 = getKeyAlgorithm(alg);
  const cryptoKey = await importPublicKey(publicKey, algorithm2);
  return await crypto.subtle.verify(algorithm2, cryptoKey, signature, data2);
}
...
var app = new Hono2();
app.use("*", cors());
app.get("/api/vps/public", vps_default.fetch);
app.use("/api/vps/nodes/*", authMiddleware);
app.use("/api/vps/add", authMiddleware);
app.use("/api/vps/update", authMiddleware);
app.use("/api/vps/delete", authMiddleware);
app.route("/api/vps", vps_default);
...
async function handleScheduled(event, env, ctx) {
  ...
  try {
    const { results: deadNodes } = await db.prepare(`
            ...
            WHERE status = 'online' 
            AND lastSeenAt < datetime('now', '-5 minutes')
        `).all();
    if (deadNodes.length > 0) {
      ...
      for (const node of deadNodes) {
        await db.batch([
          ...
          db.prepare(`
                        INSERT INTO vps_alerts (id, node_id, type, message, level, createdAt)
                        VALUES (?, ?, ?, ?, ?, ?)
                    `).bind(
            crypto.randomUUID(),
            node.id,
...
// .wrangler/tmp/bundle-1ejwdg/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-1ejwdg/middleware-loader.entry.ts
