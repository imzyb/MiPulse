...
// .wrangler/tmp/bundle-lt2s66/checked-fetch.js
...
// .wrangler/tmp/bundle-lt2s66/strip-cf-connecting-ip-header.js
...
// .wrangler/tmp/pages-uBP7Nk/functionsWorker-0.6252833002263534.mjs
...
async function login(request, env) {
  try {
    ...
    if (username === "admin" && password === adminPassword) {
      ...
      const token = await new SignJWT({ username, role: "admin" }).setProtectedHeader({ alg: "HS256" }).setIssuedAt().setExpirationTime("24h").sign(secret);
...
__name2(computeSignature, "computeSignature");
async function handleVpsRequest(path, request, env, auth) {
  ...
  if (parts[1] === "nodes") {
    if (method === "GET") {
      if (parts[2])
        return handleGetNodeDetail(parts[2], db, env);
    ...
    if (method === "DELETE" && parts[2])
      return handleDeleteNode(parts[2], db, env);
  }
  if (parts[1] === "alerts") {
    if (method === "GET")
  ...
  if (parts[1] === "targets") {
    ...
    if (method === "POST")
      return handleCreateTarget(request, db);
    if (method === "PUT" && parts[2])
...
__name2(handleVpsRequest, "handleVpsRequest");
async function handleReport(request, db, env) {
  ...
  const node = await db.prepare("SELECT * FROM vps_nodes WHERE id = ?").bind(nodeId).first();
  if (!node)
    return createError("Node not found", 404);
  if (!node.enabled)
    return createError("Node disabled", 403);
  if (signature && signatureTs) {
    const expected = await computeSignature(node.secret, node.id, signatureTs, rawBody);
    if (expected !== signature)
      return createError("Invalid signature", 401);
  } else if (node.secret !== nodeSecret) {
    return createError("Unauthorized", 401);
  }
  const report = payload.report || payload;
  const reportedAt = report.reportedAt || nowIso();
  const traffic = report.traffic || {};
  const lastReport = {
    cpuPercent: report.cpu?.usage || 0,
    memPercent: report.mem?.usage || 0,
    diskPercent: report.disk?.usage || 0,
    load1: report.load?.load1 || 0,
    traffic,
  ...
  const batch = [
    // Save report
    db.prepare("INSERT INTO vps_reports (id, node_id, reported_at, created_at, data) VALUES (?, ?, ?, ?, ?)").bind(crypto.randomUUID(), nodeId, reportedAt, nowIso(), JSON.stringify(report)),
    // Update node status
    db.prepare(`
            UPDATE vps_nodes 
            SET status = "online", 
                ...
                updated_at = ?
            WHERE id = ?
        `).bind(reportedAt, JSON.stringify(lastReport), request.cf?.country || "", nowIso(), nodeId),
    // Prune old reports (maintained for this node)
    db.prepare('DELETE FROM vps_reports WHERE node_id = ? AND reported_at < DATETIME("now", "-7 days")').bind(nodeId),
    // Global Heartbeat Cleanup
    db.prepare('UPDATE vps_nodes SET status = "offline" WHERE last_seen_at < DATETIME("now", "-10 minutes") AND status = "online"')
  ];
  if (lastReport.cpuPercent > 95 || lastReport.memPercent > 95) {
    batch.push(
      db.prepare("INSERT INTO vps_alerts (id, node_id, type, message, created_at) VALUES (?, ?, ?, ?, ?)").bind(crypto.randomUUID(), nodeId, "overload", `Node ${node.name} resources high: CPU ${lastReport.cpuPercent}%, MEM ${lastReport.memPercent}%`, nowIso())
    );
  }
  try {
    ...
    return createError("Persistence failed", 500);
  }
  return createJson({ success: true });
}
__name(handleReport, "handleReport");
__name2(handleReport, "handleReport");
async function handleListNodes(db) {
  const { results } = await db.prepare("SELECT * FROM vps_nodes ORDER BY created_at DESC").all();
  const data = results.map((row) => ({
    ...row,
    groupTag: row.group_tag,
    enabled: row.enabled === 1,
    useGlobalTargets: row.use_global_targets === 1,
    latest: row.last_report_json ? JSON.parse(row.last_report_json) : null
  }));
  return createJson({ success: true, data: { data } });
}
__name(handleListNodes, "handleListNodes");
__name2(handleListNodes, "handleListNodes");
async function handleCreateNode(request, db) {
  const body = await request.json();
  const id = crypto.randomUUID();
  const secret = body.secret || crypto.randomUUID();
  await db.prepare(`
        INSERT INTO vps_nodes (id, name, tag, group_tag, region, description, secret, status, enabled, use_global_targets, created_at, updated_at)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `).bind(id, body.name, body.tag, body.groupTag, body.region, body.description, secret, "offline", body.enabled ? 1 : 0, body.useGlobalTargets ? 1 : 0, nowIso(), nowIso()).run();
  return createJson({ success: true, id, secret });
}
__name(handleCreateNode, "handleCreateNode");
__name2(handleCreateNode, "handleCreateNode");
async function handleUpdateNode(id, request, db) {
  const body = await request.json();
  await db.prepare(`
        UPDATE vps_nodes 
        SET name=?, tag=?, group_tag=?, region=?, description=?, enabled=?, use_global_targets=?, updated_at=?
        WHERE id = ?
    `).bind(body.name, body.tag, body.groupTag, body.region, body.description, body.enabled ? 1 : 0, body.useGlobalTargets ? 1 : 0, nowIso(), id).run();
  return createJson({ success: true });
}
__name(handleUpdateNode, "handleUpdateNode");
__name2(handleUpdateNode, "handleUpdateNode");
async function handleDeleteNode(id, db) {
  await db.prepare("DELETE FROM vps_nodes WHERE id = ?").bind(id).run();
  await db.prepare("DELETE FROM vps_reports WHERE node_id = ?").bind(id).run();
  await db.prepare("DELETE FROM vps_alerts WHERE node_id = ?").bind(id).run();
  return createJson({ success: true });
}
__name(handleDeleteNode, "handleDeleteNode");
__name2(handleDeleteNode, "handleDeleteNode");
async function handleGetNodeDetail(id, db) {
  const node = await db.prepare("SELECT * FROM vps_nodes WHERE id = ?").bind(id).first();
  if (!node)
    return createError("Node not found", 404);
  const { results: reports } = await db.prepare("SELECT data FROM vps_reports WHERE node_id = ? ORDER BY reported_at DESC LIMIT 100").bind(id).all();
  const { results: targets } = await db.prepare('SELECT * FROM vps_network_targets WHERE node_id = ? OR node_id = "global"').bind(id).all();
  return createJson({
    success: true,
    data: {
      data: { ...node, lastReport: node.last_report_json ? JSON.parse(node.last_report_json) : null },
      reports: reports.map((r) => JSON.parse(r.data)),
      targets: targets.map((t) => ({ ...t, enabled: t.enabled === 1 }))
    }
  });
}
__name(handleGetNodeDetail, "handleGetNodeDetail");
__name2(handleGetNodeDetail, "handleGetNodeDetail");
async function handlePublicSnapshot(request, db) {
  const { results } = await db.prepare("SELECT id, name, tag, group_tag, region, country_code, status, last_report_json, last_seen_at FROM vps_nodes WHERE enabled = 1").all();
  const data = results.map((node) => {
    const lastReport = node.last_report_json ? JSON.parse(node.last_report_json) : null;
    return {
      ...node,
      latest: lastReport
    };
  });
  return createJson({ success: true, data });
}
__name(handlePublicSnapshot, "handlePublicSnapshot");
__name2(handlePublicSnapshot, "handlePublicSnapshot");
function handleInstallScript(request, db, env) {
  const url = new URL(request.url);
...
# ==============================================================================
# MiPulse VPS Monitor Probe - High Performance Telemetry Script
# ==============================================================================
set -e
...
echo ">>> [MiPulse] Initializing Probe for Node: $NODE_ID"

# 1. Dependency Check
for cmd in curl hostname cat grep awk; do
    if ! command -v $cmd &> /dev/null; then echo "!!! Error: Missing required command $cmd"; exit 1; fi
done

# 2. Main Metrics Collection
collect_metrics() {
    local host=$(hostname)
    ...
    # Simple traffic calc (diff is handled worker-side usually, or send raw)
    ...
    cat <<EOF > /tmp/mipulse_report.json
{
  "nodeId": "$NODE_ID",
  "secret": "$NODE_SECRET",
  "report": {
    ...
    "cpu": { "usage": $cpu },
    "mem": { "total": $mem_total, "used": $mem_used, "usage": $((mem_used * 100 / mem_total)) },
...
if [ "$1" == "--run" ]; then
    collect_metrics
    curl -s -X POST "$REPORT_URL" -H "Content-Type: application/json" -d @/tmp/mipulse_report.json
    rm /tmp/mipulse_report.json
    exit 0
fi

# 4. Automated Task Creation (Systemd / Cron)
PROBE_PATH="/usr/local/bin/mipulse-probe.sh"
cat <<EOF > $PROBE_PATH
#!/bin/bash
bash \\$0 --run
EOF
...
if command -v systemctl &> /dev/null; then
    ...
    # (Simplified for briefness, in production would create .service and .timer)
fi

echo ">>> MiPulse Probe installed successfully."
`;
  return new Response(script, { headers: { "Content-Type": "text/plain" } });
}
__name(handleInstallScript, "handleInstallScript");
__name2(handleInstallScript, "handleInstallScript");
async function handleUninstallScript(request, db) {
  const script = `#!/bin/bash
echo "Uninstalling MiPulse Probe..."
systemctl stop mipulse-probe.timer || true
rm /usr/local/bin/mipulse-probe.sh || true
`;
  return new Response(script, { headers: { "Content-Type": "text/plain" } });
}
__name(handleUninstallScript, "handleUninstallScript");
__name2(handleUninstallScript, "handleUninstallScript");
async function handleNodeConfig(request, db) {
  return createJson({ interval: 60, targets: [] });
}
__name(handleNodeConfig, "handleNodeConfig");
__name2(handleNodeConfig, "handleNodeConfig");
async function handleListAlerts(db) {
  const { results } = await db.prepare("SELECT * FROM vps_alerts ORDER BY created_at DESC LIMIT 100").all();
  return createJson({ success: true, data: { data: results } });
}
__name(handleListAlerts, "handleListAlerts");
__name2(handleListAlerts, "handleListAlerts");
async function handleClearAlerts(db) {
  await db.prepare("DELETE FROM vps_alerts").run();
  return createJson({ success: true });
}
__name(handleClearAlerts, "handleClearAlerts");
__name2(handleClearAlerts, "handleClearAlerts");
...
__name2(handleListTargets, "handleListTargets");
async function handleCreateTarget(request, db) {
  const body = await request.json();
  const id = crypto.randomUUID();
  await db.prepare(`
        INSERT INTO vps_network_targets (id, node_id, type, target, name, scheme, port, path, enabled, created_at, updated_at)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `).bind(id, body.nodeId || "global", body.type, body.target, body.name, body.scheme || "https", body.port, body.path || "/", body.enabled ? 1 : 0, nowIso(), nowIso()).run();
  return createJson({ success: true, id });
}
__name(handleCreateTarget, "handleCreateTarget");
__name2(handleCreateTarget, "handleCreateTarget");
async function handleUpdateTarget(id, request, db) {
  const body = await request.json();
  await db.prepare("UPDATE vps_network_targets SET enabled = ?, updated_at = ? WHERE id = ?").bind(body.enabled ? 1 : 0, nowIso(), id).run();
  return createJson({ success: true });
}
__name(handleUpdateTarget, "handleUpdateTarget");
__name2(handleUpdateTarget, "handleUpdateTarget");
...
__name2(handleDeleteTarget, "handleDeleteTarget");
...
__name2(handleGetSettings, "handleGetSettings");
...
// .wrangler/tmp/bundle-lt2s66/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-lt2s66/middleware-loader.entry.ts
...
//# sourceMappingURL=functionsWorker-0.6252833002263534.js.map
