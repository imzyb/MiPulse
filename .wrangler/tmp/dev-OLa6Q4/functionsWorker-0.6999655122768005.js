...
// .wrangler/tmp/bundle-JK7QZZ/checked-fetch.js
...
// .wrangler/tmp/bundle-JK7QZZ/strip-cf-connecting-ip-header.js
...
// .wrangler/tmp/pages-6acRvT/functionsWorker-0.6999655122768005.mjs
...
async function handleVpsRequest(path, request, env, auth) {
  ...
  if (parts[1] === "public" && method === "GET") {
    return handlePublicList(request, db, env);
  }
  if (!auth)
  ...
  if (parts[1] === "nodes") {
    if (method === "GET") {
      if (parts[2])
        return handleGetNodeDetail(parts[2], db);
      return handleListNodes(db);
    }
    if (method === "POST")
      return handleCreateNode(request, db);
    if (method === "PUT" && parts[2])
      return handleUpdateNode(parts[2], request, db);
...
async function handleReport(request, db, env) {
  ...
  const { nodeId, timestamp, signature, data } = payload;
  const node = await db.prepare("SELECT * FROM vps_nodes WHERE id = ?").bind(nodeId).first();
  if (!node)
    return createError("Node not found", 404);
  const isValid = await verifyPayloadSignature(node.secret, nodeId, timestamp, data, signature);
  if (!isValid)
    return createError("Invalid signature", 403);
  await db.prepare(`
    INSERT INTO vps_reports (node_id, cpu_percent, mem_percent, disk_percent, load1, uptime_sec, reported_at)
    VALUES (?, ?, ?, ?, ?, ?, ?)
  `).bind(
    nodeId,
    data.cpu?.usage || data.cpuPercent,
    data.mem?.usage || data.memPercent,
    data.disk?.usage || data.diskPercent,
    data.load?.load1 || 0,
    data.uptimeSec || 0,
    new Date(timestamp * 1e3).toISOString()
  ).run();
  await db.prepare("UPDATE vps_nodes SET last_seen_at = CURRENT_TIMESTAMP, country_code = ? WHERE id = ?").bind(data.meta?.countryCode || "", nodeId).run();
  if (data.cpu?.usage > 95 || data.mem?.usage > 95) {
    await db.prepare("INSERT INTO vps_alerts (node_id, type, message) VALUES (?, ?, ?)").bind(nodeId, "overload", `Node ${node.name} is overloaded`).run();
  }
...
async function handleListNodes(db) {
  ...
  for (const node of results) {
    node.latest = await db.prepare("SELECT * FROM vps_reports WHERE node_id = ? ORDER BY reported_at DESC LIMIT 1").bind(node.id).first();
  }
...
async function handleCreateNode(request, db) {
  ...
  await db.prepare(`
    INSERT INTO vps_nodes (id, name, tag, group_tag, region, description, secret, enabled)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?)
  `).bind(id, body.name, body.tag, body.groupTag, body.region, body.description, secret, body.enabled ? 1 : 0).run();
...
async function handleUpdateNode(id, request, db) {
  const body = await request.json();
  await db.prepare(`
    UPDATE vps_nodes SET name=?, tag=?, group_tag=?, region=?, description=?, enabled=?
    WHERE id = ?
  `).bind(body.name, body.tag, body.groupTag, body.region, body.description, body.enabled ? 1 : 0, id).run();
  return createJson({ success: true });
}
...
async function handlePublicList(request, db, env) {
  const { results } = await db.prepare("SELECT id, name, tag, group_tag, region, last_seen_at FROM vps_nodes WHERE enabled = 1").all();
  for (const node of results) {
    node.latest = await db.prepare("SELECT cpu_percent, mem_percent, disk_percent FROM vps_reports WHERE node_id = ? ORDER BY reported_at DESC LIMIT 1").bind(node.id).first();
  }
  return createJson({ success: true, data: results });
}
__name(handlePublicList, "handlePublicList");
__name2(handlePublicList, "handlePublicList");
...
async function verifyPayloadSignature(secret, nodeId, timestamp, data, signature) {
  const payloadCanonical = JSON.stringify(data);
  const dataToSign = `${nodeId}.${timestamp}.${payloadCanonical}`;
  const key = await crypto.subtle.importKey("raw", new TextEncoder().encode(secret), { name: "HMAC", hash: "SHA-256" }, false, ["verify"]);
  const isValid = await crypto.subtle.verify("HMAC", key, hexToUint8Array(signature), new TextEncoder().encode(dataToSign));
  return isValid;
}
__name(verifyPayloadSignature, "verifyPayloadSignature");
__name2(verifyPayloadSignature, "verifyPayloadSignature");
...
// .wrangler/tmp/bundle-JK7QZZ/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-JK7QZZ/middleware-loader.entry.ts
...
//# sourceMappingURL=functionsWorker-0.6999655122768005.js.map
