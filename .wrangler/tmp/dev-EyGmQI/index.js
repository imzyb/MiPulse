...
// .wrangler/tmp/bundle-RmPmsO/checked-fetch.js
...
// .wrangler/tmp/bundle-RmPmsO/strip-cf-connecting-ip-header.js
...
vps.get("/public", async (c) => {
  ...
  try {
    ...
    const { results: nodes } = await db.prepare(`
            SELECT id, name, region, country_code AS countryCode, status, last_seen_at AS lastSeenAt, tag, group_tag AS groupTag, last_report_json AS lastReport, total_rx AS totalRx, total_tx AS totalTx 
            FROM vps_nodes 
            WHERE enabled = 1 
            ORDER BY name ASC
        `).all();
...
vps.get("/nodes", async (c) => {
  const db = c.env.MIPULSE_DB;
  try {
    const { results } = await db.prepare("SELECT id, name, tag, group_tag AS groupTag, region, country_code AS countryCode, description, secret, status, enabled AS isPublic, total_rx AS totalRx, total_tx AS totalTx, last_seen_at AS lastSeenAt, last_report_json AS lastReport, created_at, updated_at FROM vps_nodes ORDER BY name ASC").all();
    const data = results.map((n) => ({
      ...n,
      latest: n.lastReport ? JSON.parse(n.lastReport) : null
    }));
    return c.json({ success: true, nodes: data });
  } catch (err) {
    return c.json({ success: false, error: err.message }, 500);
  }
});
vps.post("/add", async (c) => {
  ...
  try {
    await db.prepare(`
            INSERT INTO vps_nodes (id, name, region, enabled, secret, tag)
            VALUES (?, ?, ?, ?, ?, ?)
        `).bind(id, name, region, isPublic ? 1 : 0, finalSecret, tag || "").run();
...
vps.post("/update", async (c) => {
  ...
  try {
    await db.prepare(`
            UPDATE vps_nodes SET 
                name = ?, region = ?, enabled = ?, tag = ?, secret = ?
            WHERE id = ?
        `).bind(name, region, isPublic ? 1 : 0, tag || "", secret, id).run();
    return c.json({ success: true });
  } catch (err) {
    return c.json({ success: false, error: err.message }, 500);
  }
...
vps.get("/node/:id", async (c) => {
  ...
  try {
    const node = await db.prepare("SELECT id, name, tag, group_tag AS groupTag, region, country_code AS countryCode, description, secret, status, enabled AS isPublic, total_rx AS totalRx, total_tx AS totalTx, last_seen_at AS lastSeenAt, last_report_json AS lastReport, created_at, updated_at FROM vps_nodes WHERE id = ?").bind(id).first();
    if (!node)
...
// .wrangler/tmp/bundle-RmPmsO/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-RmPmsO/middleware-loader.entry.ts
